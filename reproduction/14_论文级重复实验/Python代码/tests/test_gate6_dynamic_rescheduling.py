...
class DynamicReschedulingTests(unittest.TestCase):
    ...
    def test_matlab_observed_dynamic_search_matches_active_event_paths(self):
        expected = {
            "order_cancellation": (1, True),
            "machine_failure": (2, False),
            "agv_failure": (2, False),
        }
        for kind, (q_columns, q_changes) in expected.items():
            with self.subTest(kind=kind):
                target = 4 if kind == "order_cancellation" else 1
                result = execute_rescheduling(
                    self.data,
                    self.chromosome,
                    self.schedule,
                    DynamicEvent(kind, 100.0, target, 15.0),
                    "RS",
                    population_size=10,
                    generations=1,
                    seed=20260818,
                    search_mode="matlab_observed",
                )
                self.assertTrue(all(len(row) == q_columns for row in result.qtable))
                initial = 0.0 if kind == "order_cancellation" else 1.0
                changed = any(value != initial for row in result.qtable for value in row)
                self.assertEqual(changed, q_changes)
                expected_evaluations = 29 if kind == "order_cancellation" else 19
