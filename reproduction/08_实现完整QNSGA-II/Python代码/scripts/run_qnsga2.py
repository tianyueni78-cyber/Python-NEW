...
def create_result(instance: str, population: int, generations: int, seed: int) -> dict:
    ...
    result = run_qnsga2(
        data, population_size=population, generations=generations, seed=seed
    )
    return {
        "算法": "QNSGA-II",
        "实例": instance,
        ...
        "Pareto染色体_MATLAB_1起始": [
            chromosome.to_matlab_row() for chromosome in result.pareto_chromosomes
        ],
        "Q表": [list(row) for row in result.qtable],
        "每代最小值": [list(row) for row in result.curve_min],
        "每代平均值": [list(row) for row in result.curve_average],
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="运行完整QNSGA-II")
    ...
    payload = create_result(
        args.instance, args.population, args.generations, args.seed
    )
