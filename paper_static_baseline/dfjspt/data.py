...
class AGVParameters:
    ...
    charging_travel_speed: float | None = None
    ...
    def validate(self) -> None:
        ...
        if self.charging_travel_speed is not None and self.charging_travel_speed <= 0:
            raise DataFormatError("充电返程速度必须为正数")
...
def load_dynamic_experiment_input(
    instance_path: str | Path,
    base_resource_path: str | Path,
    profile_path: str | Path,
    event_kind: str,
) -> ExperimentInput:
    """按三条活动动态主链加载彼此独立的资源参数。"""
    profiles = json.loads(Path(profile_path).read_text(encoding="utf-8"))
    if event_kind not in profiles:
        raise DataFormatError(f"未知动态事件资源配置：{event_kind}")
    profile = profiles[event_kind]
    instance = load_brandimarte(instance_path)
    base = load_resource_parameters(base_resource_path)
    scale = float(profile["distance_scale_from_static_coordinates"])
    resources = ResourceParameters(
        tuple(value * scale for value in base.load_to_machine),
        tuple(value * scale for value in base.machine_to_unload),
        tuple(tuple(value * scale for value in row) for row in base.machine_to_machine),
        base.load_to_unload,
        tuple(profile["machine_work_energy"]),
        tuple(profile["machine_idle_energy"]),
    ).for_machine_count(instance.machine_count)
    agv = profile["agv"]
    data = ExperimentInput(
        ...
        AGVParameters(
            int(agv["count"]),
            tuple(agv["speeds"]),
            tuple(agv["idle_energy"]),
            tuple(agv["loaded_energy"]),
            float(agv["minimum_energy"]),
            float(agv["maximum_energy"]),
            float(agv["charging_power"]),
            (
                float(agv["charging_travel_speed"])
                if "charging_travel_speed" in agv else None
            ),
        ),
    )
    data.validate()
