...
def _run(data, spec: ExperimentSpec, seed: int):
    ...
    if spec.algorithm == "nsga2":
        return run_nsga2(data, **common)
    if spec.algorithm == "moead":
        return run_moead(data, **common)
    if spec.algorithm == "mopso":
        return run_mopso(data, **common)
...
def run_batch(specs: list[ExperimentSpec], output: Path, data_root: Path) -> tuple[TrackedRun, ...]:
    if not specs:
